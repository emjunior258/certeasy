...
export const handlers = [
  http.get('/api/issuers', async ({ request }) => {
    const trap = new URL(request.referrer).searchParams.get('trap')
    if (trap === 'NO_RECORDS') return HttpResponse.json([])
    if (trap === 'DELAY') return new Promise((resolve) => {
      setTimeout(() => {
        resolve(HttpResponse.json(issuers))
      }, 3 * 1000);
    })
