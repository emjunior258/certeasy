...
class TestListIssuedCerts:
    def test_should_return_details_of_issued_tls_server_certs(self, app_container):
        CERT_SERIAL, ISSUER_ID, CERT_NAME = issue_tls_server_certs()
        PARAMS = {"type": "TLS_SERVER"}
        response = requests.get(url=f'{BASE_URL}/issuers/{ISSUER_ID}/certificates', params=PARAMS)
        assert response.status_code == 200

    def test_should_return_tls_server_as_cert_type_when_get_issued_tls_server(self, app_container):
        CERT_SERIAL, ISSUER_ID, CERT_NAME = issue_tls_server_certs()
        PARAMS = {"type": "TLS_SERVER"}
        response = requests.get(url=f'{BASE_URL}/issuers/{ISSUER_ID}/certificates', params=PARAMS)
        assert response.json()[0]["type"] == "TLS_SERVER"
    ...
    def test_should_not_return_issued_tls_server_details_when_pass_ca_type(self, app_container):
        CERT_SERIAL, ISSUER_ID, CERT_NAME, CREATE_BODY = issue_tls_server_certs()
        PARAMS = {"type": "CA"}
        response = requests.get(url=f'{BASE_URL}/issuers/{ISSUER_ID}/certificates', params=PARAMS)
        print(response.json(), CERT_SERIAL, ISSUER_ID, CERT_NAME, CREATE_BODY)
        assert len(response.json()) == 0
    ...
    def test_should_not_return_issued_tls_server_details_when_pass_custom_type(self, app_container):
        CERT_SERIAL, ISSUER_ID, CERT_NAME = issue_tls_server_certs()
        PARAMS = {"type": "CUSTOM"}
        response = requests.get(url=f'{BASE_URL}/issuers/{ISSUER_ID}/certificates', params=PARAMS)
        assert len(response.json()) == 0
