...
def test_should_not_allows_creation_of_issuer_with_empty_body(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/valid_spec.json')
    response_status = int
    # Make a request to the API
    for i in range(2):
...
def test_should_not_allows_creation_of_issuer_with_empty_name(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/name/empty_name.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
...
def test_should_not_allows_creation_of_issuer_with_null_name(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/name/null_name.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_validity_object(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/validity/empty_validity.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_validity_object(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/validity/null_validity.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_from_date(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/validity/empty_from_date.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_from_date(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/validity/null_from_date.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_until_date(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/validity/null_until_date.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_address_object(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/empty_address.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_address_object(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/null_address.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_country(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/empty_country.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_state(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/empty_state.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_state(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/null_state.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_locality(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/empty_locality.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_locality(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/null_locality.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_empty_street_address(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/empty_street_address.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_null_street_address(app_container):
    INVALID_BODY = file_reader(filename='../services/data/create_issuer/specs/invalid_body/address/null_street_address.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5
...
def test_should_not_allows_creation_of_issuer_with_sequence_of_space_from_validity(app_container):
    INVALID_BODY = file_reader(
        filename='../services/data/create_issuer/specs/invalid_body/validity/sequence_of_space_validity.json')
    # Make a request to the API
    response = requests.post(url=f'{BASE_URL}/issuers/cert-spec', json=INVALID_BODY)
    assert response.status_code == 422
    assert len(response.json()) == 5